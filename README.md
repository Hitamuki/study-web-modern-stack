...
- **GitHub & モノレポ環境構築**
  ...
  - `pnpm init` および `pnpm-workspace.yaml` (Catalogs) の作成
...
- **Hasura Actions 連携**
  ...
  - 「Hasura（受付）→ NestJS（ロジック）→ Hasura（DB保存）」のデータフロー構築
- **アジャイル・プラクティス**
...
- AWS コスト試算 & 設計調査
  ...
  - AWS Well-Architected Framework: 「コスト最適化」「信頼性」の柱を中心に、今回の構成におけるベストプラクティスを 3 つ抽出
