...
**`infra/main.tf` は書かれていましたが、一度も実行されないまま削除します。**

| コマンド | 意味 | 実施状況 |
| :- | :- | :- |
| `terraform plan` | 何をするか見せる | 実施済み |
| `terraform test` | 設定が意図どおりか検証する | 実施済み |
| `terraform apply` | **実際に作る** | **一度も未実施** |

問題は中身でした。`infra/main.tf` は **AWS の RDS** を定義していますが、
これは**起動しているだけで課金される**ため、この計画が避けようとしているものそのものです。

## だからいったん全部消します

**AWS を使う予定が無くなったため、`infra/` はディレクトリごと削除します**（層 2 / #99）。

> [!IMPORTANT]
> **Terraform をやめるわけではありません。**
> 載せ先が決まったあと、段階 3 で**無料枠のサービス向けに書き直します。**
> 消すのは AWS 向けのコードだけです。

なぜ「残しておく」ではなく「消す」なのか。

| 理由 | 内容 |
| :- | :- |
| 誤解を生む | `infra/` があると「ここを直せば公開できる」と読めてしまう |
| 手間が増える | 使わないのにバージョン更新や静的解析の指摘が発生する |
| 消えない | Git の履歴には残るので、学習の記録は失われない |
