...
# state の置き場所（暫定でローカル / 2026-09-06 決定）

**当面はローカル state にする。** つまり **CI から `terraform apply` を流さない。**
下表の「ローカルのまま」を選んだということであり、**自動化をいったん諦める**という意味である。

無料枠の条件（特に HCP Terraform の資源数上限）が未検証のままで、
検証コストに見合う段階ではないと判断した。**必要になった時点で下表から選び直す。**

**削除前の `infra/` もローカル state だった**（バックエンド設定が無い）。
...
| **ローカルのまま（採用 / 暫定）** | CI から `apply` しない。人間が手元で流す | 自動化を諦めることになる |

> [!NOTE]
> 上表の無料枠の条件は**未検証のまま**である。リモート state に移す判断をするときに実査し、
> 数値の正本は Discussion #29 に確認日付つきで記録する（[decision.md](/project/plan/deploy/decision.md)）。
