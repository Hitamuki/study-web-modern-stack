...
「廃止した ID は再利用しない」の唯一の例外です。**このルール自体は今後も維持します。**
再採番したくなったら、まずこの表に前例として並べられる状況かを確認してください。

| 日付 | 内容 | 理由 | Issue |
| :- | :- | :- | :- |
| 2026-08-17 | `SCR-001` をダミー画面からログインへ付け替え。ダミー画面を `SCR-005` へ移し、認証 4 画面を `SCR-001`〜`SCR-004` に詰めた | アプリの入口はログインであり、ダミー画面は実ドメインが決まるまでの仮置き。**未デプロイで外部依存が無いうち**に順序を整えた | #68 |
